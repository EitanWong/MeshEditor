...
using UnityEditor;
using UnityEngine;
using UnityExtensions.MeshPro.MeshEditor.Editor;
using UnityExtensions.MeshPro.MeshEditor.Editor.Scripts.Base;
using UnityExtensions.MeshPro.MeshFilterProEditor.Editor.Enum;

/// <summary>
/// 网格选择器扩展
/// </summary>
public class MeshEditPage : MeshEditorPage
{
...
#pragma warning restore 414
    ...
    private void OnInit()
    {
       ...
       objectGUIContant = new GUIContent("对象模式", Resources.Load<Texture2D>("Textures/Object"), "对象模式");
       vertexGUIContant = new GUIContent("顶点模式", Resources.Load<Texture2D>("Textures/Vertex"), "顶点模式");
       edgeGUIContant = new GUIContent("线框模式", Resources.Load<Texture2D>("Textures/Edge"), "线框模式");
       faceGUIContant = new GUIContent("面片模式", Resources.Load<Texture2D>("Textures/Face"), "面片模式");
       finishGUIContant = new GUIContent("完成编辑", Resources.Load<Texture2D>("Textures/Finish"), "完成编辑");
       cancelGUIContant = new GUIContent("取消编辑", Resources.Load<Texture2D>("Textures/Cancel"), "取消编辑");
       editGUIContant = new GUIContent("简单编辑", Resources.Load<Texture2D>("Textures/Edit"), "简单编辑");
       saveGUIContant = new GUIContent("保存网格数据", Resources.Load<Texture2D>("Textures/Save"), "保存网格数据");
       MeshEditPageIcon = Resources.Load<Texture2D>("Textures/MeshEdit");
    ...
    private void DrawEditModeGUIButton()
    {
        if (GUILayout.Button(IsEdit ? cancelGUIContant : editGUIContant))
        {
            ...
            else
            {
                ...
                {
                    MeshEditorUtility.ShowNotificationOnSceneView("取消退出", 1f);
                    return;
                }
            }
        ...
        if (GUILayout.Button(objectGUIContant))
        {
            ...
            MeshEditorUtility.ShowNotificationOnSceneView("对象模式", 1f);
        }

        if (GUILayout.Button(vertexGUIContant))
        {
            current_EditMode = MeshEditMode.Vertex;
            MeshEditorUtility.ShowNotificationOnSceneView("顶点模式", 1f);
        }

        if (GUILayout.Button(edgeGUIContant))
        {
            current_EditMode = MeshEditMode.Edge;
            MeshEditorUtility.ShowNotificationOnSceneView("线框模式", 1f);
        }

        if (GUILayout.Button(faceGUIContant))
        {
            current_EditMode = MeshEditMode.Face;
            MeshEditorUtility.ShowNotificationOnSceneView("面片模式", 1f);
        }
        ...
        switch (current_EditMode)
        {
            ...
            case MeshEditMode.Edge:
                ...
                Handles.DrawLine(_mouseHoverFace.m_edge1.m_vertex1.m_vertex,
                    _mouseHoverFace.m_edge1.m_vertex2.m_vertex, 1f);
                Handles.DrawLine(_mouseHoverFace.m_edge2.m_vertex1.m_vertex,
                    _mouseHoverFace.m_edge2.m_vertex2.m_vertex, 1f);
                Handles.DrawLine(_mouseHoverFace.m_edge3.m_vertex1.m_vertex,
                    _mouseHoverFace.m_edge3.m_vertex2.m_vertex, 1f);
                break;
            case MeshEditMode.Face:
                Handles.DrawSolidRectangleWithOutline(_mouseHoverFace.m_faceVertex, Color.red, Color.blue);
                break;
        }
    }
    ...
    private void ToolStateCheck()
    {
        switch (Tools.current)
        {
            case Tool.Move:
                ...
                MeshEditorUtility.ShowNotificationOnSceneView("移动工具", 0.5f);
                break;
            case Tool.Rotate:
                current_EditTool = MeshEditTool.Rotate;
                MeshEditorUtility.ShowNotificationOnSceneView("旋转工具", 0.5f);
                break;
            case Tool.Scale:
                current_EditTool = MeshEditTool.Scale;
                MeshEditorUtility.ShowNotificationOnSceneView("缩放工具", 0.5f);
                break;
        }
    ...
    private void EdgeEditHighLight()
    {
        ...
        {
            Handles.DrawLine(edge.m_vertex1.m_vertex, edge.m_vertex2.m_vertex, 3f);
        }
    }
    ...
    private void FaceEditHighLight()
    {
        ...
        {
            Handles.DrawSolidRectangleWithOutline(face.m_faceVertex, Color.magenta, Color.yellow);
        }
    }
