...
public class MeshVoxelizerPage : MEDR_Page
{
    ...
    private void MeshVoxelizerMenu()
    {
        ...
        // {
        _config.MEDR_MeshVoxelizer_PixelSize = EditorGUILayout.IntField(new GUIContent("像素大小"), _config.MEDR_MeshVoxelizer_PixelSize);
        ...
        {
            _config.MEDR_MeshVoxelizer_CustomVoxelMat = EditorGUILayout.Toggle(new GUIContent("自定义体素材质"), _config.MEDR_MeshVoxelizer_CustomVoxelMat);
        }
        ...
        if (GUILayout.Button("创建体素网格"))
        {
            for (int i = 0; i < CheckFields.Count; i++)
            {
                ...
                var voxelMesh = VoxelMeshBuilder.ConversionVoxelMesh(editMeshFilter.sharedMesh, _config.MEDR_MeshVoxelizer_PixelSize);
                if (voxelMesh)
                {
